...
# dfRead = pd.read_excel('科目余额表_2025年9月-2025年9月_广州白云恒利包装材料有限公司_20251103.xls', sheet_name='科目余额表')
# print(dfRead)
# dfRead.to_excel('./测试输出.xls', sheet_name='员工信息', index=False)
...
# wb.save('科目余额表_2025年9月-2025年9月_广州白云恒利包装材料有限公司_20251103_modified14.xls')
# wb.close()
# app.quit()
...
# 2、算法：
# 拿出每一个单元格里面的值，
# 如果这个值前面没有空格，那么他是第一级，存入
# ['应交税费']
# 如果这个值前面有两个空格，那么他是第二级，存入
# ['应交税费','应交税费—@@@—应交增值税']
# 如果这个值前面有四个空格，那么他是第三级，存入
# ['应交税费','应交税费—@@@—应交增值税','应交税费—@@@—应交增值税—@@@—进项税额']
#['应交税费','应交税费—@@@—应交增值税','应交税费—@@@—应交增值税—@@@—转出未交增值税']
...
['应交税费','应交税费—@@@—应交增值税']
['应交税费','应交税费—@@@—应交增值税','应交税费—@@@—应交增值税-@@@-转出未交增值税']
['应交税费','应交税费—@@@—应交增值税','应交税费—@@@—应交增值税—@@@—减免税款']
['应交税费','应交税费—@@@—应交增值税','应交税费—@@@—应交增值税—@@@—销项税额']
